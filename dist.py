...
def save_each_file(base_path, file_name, data):
    ...
    with open(static_dir, 'w') as f:
        ...
        f.write(json_str)
    print(colored(f"Save {dist_rank} gpu's {file_name} save complete", "light_red", "on_yellow"))    
    # sync
    if get_world_size() > 1:
        dist.barrier()
...
def merge_meta_files(file_paths, output_path):
    ...
    with open(output_path, 'w') as f:
        ...
        print(colored(f"Save integration meta information ver. save here: {output_path}", "light_red", "on_yellow"))    
